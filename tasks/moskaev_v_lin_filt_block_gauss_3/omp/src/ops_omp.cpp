...
MoskaevVLinFiltBlockGauss3OMP::MoskaevVLinFiltBlockGauss3OMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType();
}

bool MoskaevVLinFiltBlockGauss3OMP::ValidationImpl() {
  const auto &input = GetInput();
  return !std::get<4>(input).empty();
}

bool MoskaevVLinFiltBlockGauss3OMP::PreProcessingImpl() {
  return true;
}

void MoskaevVLinFiltBlockGauss3OMP::ApplyGaussianFilterToBlock(const std::vector<uint8_t> &input_block,
...
bool MoskaevVLinFiltBlockGauss3OMP::RunImpl() {
  const auto &input = GetInput();
...
bool MoskaevVLinFiltBlockGauss3OMP::PostProcessingImpl() {
